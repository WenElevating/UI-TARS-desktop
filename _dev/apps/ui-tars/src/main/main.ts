...
const loadDevDebugTools = async () => {
  ...
  import('electron-devtools-installer')
    .then((mod) => {
      ...
      const { REACT_DEVELOPER_TOOLS } = mod as unknown as {
        REACT_DEVELOPER_TOOLS?: unknown;
      };

      if (typeof installExtension !== 'function') return;
      if (!REACT_DEVELOPER_TOOLS) return;

      return Promise.all([
        (installExtension as (ext: unknown) => Promise<string>)(
          REACT_DEVELOPER_TOOLS,
        ),
      ])
        .then((names) => logger.info('Added Extensions:', names.join(', ')))
        .catch((err) => logger.error('An error occurred adding extension:', err));
    })
    .catch(logger.error);
};
...
const initializeApp = async () => {
  ...
  if (env.isMacOS) {
    ...
    const { ensurePermissions } = await import('./utils/systemPermissions');
